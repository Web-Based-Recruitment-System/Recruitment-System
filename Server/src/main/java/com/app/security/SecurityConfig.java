...
//	}
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
	    http
	        ...
	        .authorizeRequests(auth -> auth
	            // 3. Public Endpoints
	            .antMatchers("/users/signup", "/users/signin", "/users/send-otp", 
	                         "/users/reset-password", "/users/verify-otp").permitAll()
	            .antMatchers("/v*/api-doc*/**", "/swagger-ui/**").permitAll()
	            
	            // 4. THE FIX FOR PREFLIGHT: Permit all OPTIONS
	            .antMatchers(HttpMethod.OPTIONS, "/**").permitAll()
	            
	            // 5. Role Based Access (Fixed the skill path quote error)
	            .antMatchers("/admin/**").hasRole("ADMIN")
	            .antMatchers("/hr/**").hasRole("HR")
	            .antMatchers("/applicant/**", "/job/**", "/skill/**").hasRole("APPLICANT")
	...
	// Separate Bean for CORS to ensure it's picked up early
	@Bean
